...
bool RECEIVE_ATTR RCSwitch::receiveProtocol(const int p, unsigned int changeCount) {
    ...
    DBG("Change count",changeCount);
    DBG("Tolerance",delayTolerance);
    DBG("pro.zero.high",delay*pro.zero.high);
    DBG("pro.zero.low",delay * pro.zero.low);
    DBG("pro.one.high", delay*pro.one.high);
    DBG("pro.one.low", delay * pro.one.low);
        
    for (unsigned int i = firstDataTiming; i < changeCount - 1; i += 2) {


      DBG("\n0A",diff(RCSwitch::timings[i], delay*pro.zero.high));
      DBG("0B",diff(RCSwitch::timings[i + 1], delay * pro.zero.low));
      DBG("1A",diff(RCSwitch::timings[i], delay * pro.one.high));
      DBG("1B",diff(RCSwitch::timings[i + 1], delay * pro.one.low));
...
//Serial.println("Hello==========");
    ...
    {
    // Serial.println(RCSwitch::timings[i]);
    }
   // Serial.println(diff(duration, RCSwitch::timings[0]) );
    ...
    if (diff(duration, RCSwitch::timings[0]) < 200) {
      ...
      repeatCount++;
      if (repeatCount == 2) {
        DBG("Testing protocols #",numProto);
